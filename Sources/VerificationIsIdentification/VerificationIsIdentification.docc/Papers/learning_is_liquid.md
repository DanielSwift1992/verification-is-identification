...
In Permanence Is Resistance the author derived the crystal (the saturated library, permanent and irreducible) and named its complement only in passing: a *liquid* phase below saturation that, unlike a physical melt, never melts. Here the author derives the liquid. The un-saturated regime is not a deficient crystal. It is a phase with its own dynamics, and for most domains it is the permanent one: a horn never crystallizes its reach, only its basis, so the reach is liquid forever.

The liquid has two coupled dynamics. A **crystallization front** (the frontier) advances monotonically, no melting, a one-sided free boundary whose speed is the dual flow's deposition rate and whose geometry is the W-metric: this is the discriminator side. A **search flow** (the generator descending the loss) is *laminar* where the landscape is aligned and *turbulent* where it is rugged, its roughness measured by the loss-landscape autocorrelation: this is the generator side. Composition is the nonlinear cascade that carries a finite basis across scales into the reach, with rate γ and free energy the self-play partition function.

The two couple exactly as a Stefan problem couples a diffusion field to its freezing interface: search finds, the gate freezes, the new crystal reshapes the field, the search shifts, monotone throughout, a supercooled liquid that only solidifies. Where a region is still liquid the only honest output is a guess, and at the front, a certificate. The gate is the membrane between them, and current statistical AI is pure liquid with neither front nor membrane.

The mathematics is borrowed as **tools** (free-boundary analysis for the front, spectral ruggedness for the search, the partition function for the cascade), and the boundary against claiming the *equations* of physics, the heat equation, Navier–Stokes, any Millennium Problem, is stated outright. Knowledge is solid, and learning is liquid.

## §0. Derivation

No new postulate. The author derives everything below from the two phases (PiR §3), the frontier (DiC §4), the dual flow (InI S6a–S6c), append-only accumulation (I3), clustering (DiC G11–G12), and alignment (GA).
...
2. A horn domain (B8b: finite basis, infinite reach) never crystallizes its reach: only the basis crystallizes, and the reach is derived on demand (B6–B7). So below-saturation is not transitional but permanent. *[B8b]*
...
4. The solid region only grows (I3: a certified fact never un-certifies), a one-sided moving boundary, and the generator flows through the liquid guided by loss (GA, DiC G13). Two coupled dynamics: a freezing front and a searching flow. *[I3; GA; DiC G13]*

PiR derived where learning *ends*. Here the author derives the medium learning *is*.

## §1. The Liquid Is a Phase, Not a Deficit

**LQ1 (Liquid is the generic state).** For a horn (infinite reach, B8b) or a prime-like domain (infinite basis, B8a), full crystallization never completes, and the agent operates permanently below saturation. The complete crystal is the special case of a finite closed domain.
...
*Consequence.* The crystal endpoint (PiR) is rare, and the liquid is the norm. "Learning" names operation in this phase, and it has structure of its own.

**LQ2 (Saturation is a field, not a scalar, and crystallization is local).** The saturation state is inhomogeneous: clustered sub-domains crystallize first (their W-balls fill cheaply, G11), while metrically-uniform regions are still liquid. The library is solid patches in a liquid surround, with levels of saturation that vary by region.

*Proof.* G11: a region of covering number N(r) anchors in N(r)·H_{N(r)} tasks, and a clustered region (N(r) ≪ K) crystallizes far sooner than a flat one (N(r) = K). So the solid fraction is a field over the domain, dense where the W-matrix clusters (G12), sparse where it is flat. QED.
...
QED. *(Structural identification: the shared structure is (one-sided, rate-bounded, history-dependent position). The Stefan analogy is borrowed as a tool (§6), and the author derives no heat equation.)*
...
*Consequence.* Where the domain clusters, the solid grows in branched fronts (cheap, fast), and where it is flat, the front is a featureless wall that advances only by exhaustive coverage. The shape of learning's freezing is the shape of the domain's W-geometry.
...
**LQ7 (Composition is the nonlinear cascade, and γ is its rate).** Composition (B1) carries a finite atom set A across depths into the reach: Cl_d(A) grows with d (B2), and its growth rate γ = lim sup_s (ln N(s))/s (B8d) measures how fast the reach expands per unit norm.
...
(iii) *Reshape.* The new crystal entry changes the loss landscape: with more cached structure, the loss of nearby candidates drops (they can be composed from the new entry rather than derived from scratch, InI S6c: self-thickening). The field is reshaped by the boundary's advance.
...
**LQ10 (Guess in the liquid, certificate at the front, and the gate is the membrane).** In a still-liquid region (uncertified) the only output the generator can offer is a guess, uncertified, possibly wrong. Past the front (crystallized) the output is a certificate. The gate is the membrane between them: certified answer out, honest silence (|S| > 1 / EMPTY) where still liquid (answer-or-silence, PP7, PP15). Current statistical AI is **pure liquid with neither front nor membrane**: no I3 (the freeze never sticks, catastrophic forgetting, IiI §3), no gate (guesses leak out as if certified, hallucination). It flows everywhere (its strength) and solidifies nowhere (its fragility).

*Proof.* PP7: the certified guarantees fail without I3 (nothing keeps the front) or the gate (no membrane). A system with neither is all flow: it generalizes (laminar where α > 0) but cannot certify, cannot retain, cannot abstain. QED.
...
- *Free boundary / Stefan* for the front (LQ3, LQ8): a monotone moving solid/liquid interface coupled to a field. The author does **not** derive the heat equation.
- *Spectral ruggedness / turbulence* for the search (LQ5–LQ6): roughness across scales, autocorrelation as spectrum. The author does **not** model fluid turbulence and makes **no** claim on Navier–Stokes existence or smoothness: that the *question type* (does a field stay smooth or develop singular roughness?) rhymes with α-smoothness vs ruggedness is a rhyme, not a reduction, and the Millennium Problem is untouched.
...
4. The search is laminar where aligned, turbulent where rugged, ~1/α is its Reynolds number, and the loss autocorrelation is its spectrum (LQ5–LQ6).
5. Composition is the nonlinear cascade carrying basis into reach at rate γ, free energy the partition function (LQ7).
6. Field and front couple as a Stefan problem, and the liquid is supercooled: it only freezes (LQ8–LQ9).
...
8. The mathematics is borrowed as tools, and the author claims no equation of physics (§6).

> **Knowledge is solid, and learning is liquid. A front that freezes and never melts, a search that runs laminar or turbulent by how aligned the world is, and the only honesty in the liquid is to call a guess a guess until the front reaches it. The crystal is the end of the story, and the liquid is the story.**
...
In Permanence Is Resistance the author derived the solid that learning deposits. Here the author derives the medium it deposits from: a freezing, searching, supercooled liquid whose front is the discriminator and whose flow is the generator, the same two organs, now in motion. The crystal is matter, the liquid is the becoming of matter, and the gate is the only thing that tells one from the other.
