...
| 1 | Library L (K rule classes) | A lookup table: task type → solution method |
...
1. Fixed target: L_N is permanent and complete → satisfied (I3 + Saturation Theorem).
2. Candidate space: structures in Σ that are not L_N → satisfied if |Σ| > |L_N| (Σ is non-trivial).
3. Decidable test: COMPARE on Σ → satisfied (I2, defined on all of Σ).
...
The tower is bottom-up. Level 0 saturates → L₀ fixed → level 1 ignites → saturates → L₁ fixed → level 2 ignites → ... → tower terminates (V=I §5.16).
...
|-------|-------------|--------|
| I1 (finite) | dim(H) < ∞ → finite set of measurement outcomes | Yes (given finite dim) |
...
Where axioms fail: hydrogen disorder → |S| > 1 on hydrogen positions. V=I diagnostic: the oxygen encoding has |S| = 1, the hydrogen encoding is insufficient.
