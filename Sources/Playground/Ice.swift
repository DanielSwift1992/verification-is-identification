...
// ═══════════════════════════════════════════════════════════════
// The ice rule at the TYPE LEVEL — the residual entropy of ice, settled by
...
/// Two: the framework's `IntegerValued` ladder at the value the ice rule pins.
///
...
/// A vertex's bonds carry a running in-count the type checker reduces. The count
...
/// No bonds remain: the in-count floors at `Never`, the zero of the ladder. [Matter §5]
public enum Vacuum: Bonds { public typealias InCount = Never }

/// A proton pointing IN lifts the running count by one step (`Succ`). [Matter §5]
public struct In<Rest: Bonds>: Bonds { public typealias InCount = Succ<Rest.InCount> }

/// A proton pointing OUT leaves the running count where it was. [Matter §5]
public struct Out<Rest: Bonds>: Bonds { public typealias InCount = Rest.InCount }
...
extension In:  IceVertex where InCount == Two {}
extension Out: IceVertex where InCount == Two {}
...
public enum Survivor<Vertex: IceVertex> {}
