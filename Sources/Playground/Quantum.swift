...
/// One: the eigenspace dimension a non-degenerate eigenvalue collapses to. [Matter §5]
...
/// The empty space: the dimension floors at `Never`, the zero of the ladder. [Matter §5]
public enum NoEigenstate: Eigenspace { public typealias Dimension = Never }

/// One more eigenstate sharing the measured eigenvalue lifts the dimension by one. [Matter §5]
public struct Eigenstate<Rest: Eigenspace>: Eigenspace { public typealias Dimension = Succ<Rest.Dimension> }

/// A measurement IDENTIFIES exactly when the eigenvalue is non-degenerate — a
...
extension Eigenstate: IdentifyingMeasurement where Dimension == One {}

/// An identification certified by the type checker: it forms only over a
/// non-degenerate eigenvalue. [Matter §5]
public enum Identified<Measurement: IdentifyingMeasurement> {}
