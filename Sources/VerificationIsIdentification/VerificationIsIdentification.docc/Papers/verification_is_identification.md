...
**On uniqueness.** This is the first of four places where the system turns on exactly one of something, and they are worth reading together. Uniqueness of the **answer** is here: |S| = 1, and passing is identification (step 11, Theorem 2). Uniqueness of the **primitive** is §2.5: PAIR is the sole structural primitive, and NULL, COMPARE and projection are aspects of it rather than entities beside it. Uniqueness of the **source** is §5.21: COMPARE alone produces a new equality classification; everything else carries information in or takes it apart. Uniqueness of the **aligner** is the remark to §5.25: I3 alone forces every monotone axis to co-orient, and removing it misaligns them all. The four are not analogies of one another — each is derived where it stands — but a reader who has seen one has seen the shape of the others, and a builder who has seen them as one principle has a test for every new entity they are tempted to introduce.
...
**Theorem 4 (Universality).** For any finite deterministic task, there exists a semantics-preserving encoding and example set producing an R4-valid game. The framework requires decidable verification over the encoding, and the agent must choose the encoding so that verification reduces to structural comparisons.
