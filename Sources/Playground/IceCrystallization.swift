...
// ═══════════════════════════════════════════════════════════════
// A DOMAIN names crystallization of ice. [Matter §5]
//
...
public struct IceOxygen: SystemCrystallizes {
    ...
    public typealias Left        = Cell<
        Never,
        Never
    > // the configuration pair COMPARE reads
    ...
    public typealias Cache       = Grew<
        Never,
        Never
    > // excluded configs ACCUMULATE, append-only (I3, Boltzmann)
...
public struct IceHydrogen {
    public typealias SolutionSet = Cell<
        Three,
        Never
    > // |S| > 1: the ice-rule manifold (Pauling)
}
