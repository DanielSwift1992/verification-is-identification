...
// ═══════════════════════════════════════════════════════
// TreeSort — induce the documentation tree from the type lattice, in Swift.
//
...
struct Symbol: Decodable {
    ...
    struct Fragment: Decodable { let kind: String; let spelling: String; let preciseIdentifier: String? }
    struct Location: Decodable {
        struct Position: Decodable { let line: Int }
        let uri: String?; let position: Position?
    }
...
struct Relationship: Decodable { let kind: String; let source: String; let target: String }
...
for (i, s) in G.symbols.enumerated() { sym[s.identifier.precise] = s; index[s.identifier.precise] = i }
...
while changed {
    changed = false
    for p in protos where !axis.contains(p) && !SEED.contains(p) && opens.contains(p) {
        if par[p]!.isEmpty || par[p]!.allSatisfy({ axis.contains($0) }) { axis.insert(p); changed = true }
    }
}
...
func anc(_ p: String) -> Set<String> {
    ...
    var out = Set<String>()
    for q in par[p] ?? [] { out.insert(q); out.formUnion(anc(q)) }
    ancCache[p] = out; return out
}
...
for p in protos where cls[p] == "theorem" { theoremsOn[realClosure(p).first!]!.append(p) }
...
func bullets(_ ns: [String]) -> String { ns.map { "- ``\($0)``" }.joined(separator: "\n") }
...
func paperTopics() -> [String: String] {
    ...
    for p in protos where cls[p] == "bridge" {
...
func parseGroups(_ md: String) -> [String: [String]] {
    var groups: [String: [String]] = [:]; var cur: String?
    ...
    for raw in body.components(separatedBy: "\n") {
        ...
        if let m = headRe.firstMatch(in: raw, range: NSRange(location: 0, length: ns.length)) {
            cur = ns.substring(with: m.range(at: 1)).trimmingCharacters(in: .whitespaces); groups[cur!] = []
