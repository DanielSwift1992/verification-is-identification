...
*Proof.* V=I §5.24: the pipeline is a pure function of the encoded input. No external state is referenced. A transformation that preserves the encoding produces the same pipeline input $\to$ same output $\to$ same perspective. No second observer exists to register the transformation. QED

This is gauge invariance from the agent side: any transformation that doesn't change what the agent sees is undetectable — not because it's hidden, but because detection requires a second perspective.
...
Same target, different encoding $\to$ different features visible $\to$ different f* (V=I §5.4: encoding-local).
...
The human visual system: three cone types (S, M, L). Each cone type is an encoding sensitive to a different frequency band. Three encodings applied simultaneously $\to$ three independent COMPARE streams $\to$ three-dimensional quality space.
...
Color blindness = fewer independent encodings. Richer perception = more independent encodings. The dimensionality is not in the target — it's in the agent's encoding apparatus.
...
Three axes $\to$ three "directions" the agent can turn. Each direction reveals a different aspect of the target.
...
A richer grammar $\to$ more directions $\to$ higher-dimensional space. A minimal grammar $\to$ fewer directions $\to$ lower-dimensional space. The grammar is designed by the agent (V=I §6) or inherited from the level below (Matter §4).

## §4. Depth: Tower Saturation

The tower (Matter §4, M8–M16) has height M. At each level, the agent either:
- Has saturated $\to$ lookup O(K×F)+1 $\to$ "sees" this level instantly
- Has not saturated $\to$ brute force O(T_brute) $\to$ "works" through this level

**S6 (Depth of vision).** The agent's depth of vision = the height of the saturated portion of its tower. A fully saturated tower (M16: K_M $\le$ 1) = maximum depth. An unsaturated tower = shallow vision — the agent sees the current level but must derive everything deeper.
...
- d(A₁, A₃) $\le$ d(A₁, A₂) + d(A₂, A₃) (triangle inequality: the COMPAREs A₁ is missing relative to A₃ are bounded by the sum of what's missing through A₂)
- d is bounded: d $\le$ |Σ|² (M11: maximum COMPAREs)

This is a metric. It satisfies all metric axioms. The space of all possible agent states under this metric is the agent's geometry.
...
One agent $\to$ no distance $\to$ no space (S1).
Two agents $\to$ one distance $\to$ a line.
Three agents with independent caches $\to$ three distances $\to$ a plane (if linearly independent).
n agents $\to$ up to (n-1)-dimensional space.
...
**S9 (Translation).** Finding a mapping m: E₁ $\to$ E₂ such that f*(E₁) maps to f*(E₂) is itself a V=I game at the encoding level (Matter §4, level 2). The cost of communication between agents with different encodings = the cost of meta-saturation between their encoding classes.
...
**S12 (Stillness).** Fixed encoding = no change in perspective = stillness. The agent may still run COMPAREs (time passes, M1), but the encoding doesn't change $\to$ the quality of perception doesn't change $\to$ spatial position (relative to grammar axes) is fixed.
...
*Proof.* Each independent axis = one direction (S4). Independence of axes = independence of directions (changing along one axis does not change along another). d axes $\to$ d independent directions $\to$ d-dimensional space. QED
...
**Theorem S17 (Depth is bounded).** Maximum perceptual depth = M levels, where M $\le$ K₀ (M16: tower termination).
...
This is not c in meters per second. It is 1 in (feature-distance / COMPARE-tick). The conversion to physical units requires the specific grammar of physical measurement. But the existence of a maximum speed is structural: no agent can change its perspective faster than one distinction per tick, because each distinction requires its own COMPARE.
...
In physics: Δx · Δp $\ge$ ℏ/2. Conjugate variables cannot both be precisely determined.
...
Δ(position encoding) + Δ(momentum encoding) $\ge$ f_x + f_p − F
