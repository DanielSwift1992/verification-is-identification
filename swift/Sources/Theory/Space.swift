// ═══════════════════════════════════════════════════════════════
// Difference Is Distance (Space) — paper 4.
//
...
/// grammar — a finite count, a property of the grammar (``HasEncoding``), not of
/// the target or the agent. [DiD S5, S14]
...
/// has no second frame, so any encoding-preserving transform is undetectable —
...
/// Distance between two agents on the same target is the symmetric difference of
/// their caches, `|G₁ △ G₂|` — a metric: identity, symmetry, triangle, bounded
/// `≤ |Σ|²`. ``HasDistance`` over ``HasCache``, with `Distance`, already a finite
/// count (``IntegerValued``), here pinned ``MetricProperty``. [DiD S7]
///
...
/// distance, S1); n agents span up to `(n−1)` dimensions. A metric
