...
enum GenerateOrg {
    ...
    static func run(_ args: [String]) {
        ...
        // ── VectorDemo: the walk film — the phone-book walk to one named person, frame per
        //    step, faces as NameSpan witnesses, cues stated for SVG's own clock. Generated
        //    beside the walk because the shelves shift with the roster and the film follows.
        var film = ["import VerificationIsIdentification", "import DocumentKit", "import Organization", "",
        ...
        var filmSteps: [(word: String, left: String, right: String, tookLeft: Bool)] = []
        let aliceSpot = shelf.indices.first { shelf[$0].reference == "Alice" }
        if let aliceIndex = aliceSpot {
            var lo = 0
            var hi = shelf.count - 1
            while lo < hi {
                let half = (hi - lo + 1) / 2
                let leftFace = "NameSpan<\(shelf[lo].reference), \(shelf[lo + half - 1].reference)>"
                let rightFace = "NameSpan<\(shelf[lo + half].reference), \(shelf[hi].reference)>"
                let tookLeft = aliceIndex <= lo + half - 1
                let word = filmSteps.map { $0.tookLeft ? "WentLeft" : "WentRight" }
                filmSteps.append((word: (["WordSoFar"] + word.flatMap { [$0, "StepDot"] }.dropLast()).joined(separator: ".self; ") + ".self",
                                  left: leftFace, right: rightFace, tookLeft: tookLeft))
                if tookLeft { hi = lo + half - 1 } else { lo = lo + half }
            }
        }
        let filmDwell = 2
        for (k, step) in filmSteps.enumerated() {
            let wordBody = filmSteps.isEmpty ? "WordSoFar.self" : step.word
            film += ["enum FilmWord\(k): Fragment {",
                     "    @StructureBuilder",
                     "    static var body: some Structure {",
                     "        \(wordBody)",
                     "    }",
                     "}"]
            let art = step.tookLeft ? "FilmStepLeft" : "FilmStepRight"
            film += ["typealias FilmFrame\(k) = TimedReveal<FilmCue\(k), FilmDwell, \(art)<FilmWord\(k), \(step.left), \(step.right)>>",
                     "enum FilmCue\(k): Close {}",
                     "extension FilmCue\(k) {",
                     "    static var typeName: String { \"\(k * filmDwell)\" }",
                     "}", ""]
        }
        film += ["enum FilmDwell: Close {}",
                 "extension FilmDwell {",
                 "    static var typeName: String { \"\(filmDwell)\" }",
                 "}",
                 "enum FilmCueEnd: Close {}",
                 "extension FilmCueEnd {",
                 "    static var typeName: String { \"\(filmSteps.count * filmDwell)\" }",
                 "}",
                 ...
                 "    static var typeName: String {",
                 "        \"The phone-book walk playing itself to Lovelace, Alice: \(filmSteps.count) choices, \"",
                 ...
                 "    static var body: some Structure {",
                 "        FilmGroundRow.self"]
        for k in 0..<filmSteps.count {
            film.append("        FilmFrame\(k).self")
        }
        film += ["        FilmFinale.self",
                 "    }",
                 "}",
                 ...
                 "    print(\"generated 1 walk film (\(filmSteps.count) frames + finale, SVG clock)\")",
                 "}"]
        try? (film.joined(separator: "\n") + "\n").write(toFile: "Sources/VectorDemo/GeneratedWalkFilm.swift", atomically: true, encoding: .utf8)
