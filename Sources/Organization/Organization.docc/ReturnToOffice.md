...
> Warning: The third arrangement is not only listed as "no". Compile ``Carol`` as office-compliant and the build refuses: her `Site` is ``Remote``, and the office-minimum conformance has no instance over `Remote` to satisfy. The refusal names the rule, not just the person.
