...
// ═══════════════════════════════════════════════════════════════
// The hydrogen spectrum by the selection rule — a real spectroscopy question,
...
/// Orbital angular momentum on the ladder: s = 0, p = 1, d = 2.
...
public protocol DipoleAllowed { static var line: String { get } }

/// The Δℓ = −1 branch: the orbital drops one step.
public enum Lowers<From: Orbital, To: Orbital> {}
extension Lowers: DipoleAllowed where From.L == Succ<To.L> {
    public static var line: String { "\(From.label) → \(To.label)" }
}

/// The Δℓ = +1 branch: the orbital rises one step.
public enum Raises<From: Orbital, To: Orbital> {}
extension Raises: DipoleAllowed where To.L == Succ<From.L> {
    public static var line: String { "\(From.label) → \(To.label)" }
}

/// The lines the compiler admits — each forms only because its Δℓ reduces to ±1.
public let hydrogenLines: [DipoleAllowed.Type] = [
    Lowers<Orbital2p, Orbital1s>.self,   // Lyman, Δℓ = −1
    Lowers<Orbital3p, Orbital1s>.self,   // Lyman, Δℓ = −1
    Raises<Orbital3s, Orbital2p>.self,   // Balmer, Δℓ = +1
    Lowers<Orbital3p, Orbital2s>.self,   // Balmer, Δℓ = −1
    Lowers<Orbital3d, Orbital2p>.self,   // Balmer, Δℓ = −1
]

// Build with -DSHOW_FORBIDDEN to make the compiler refuse the forbidden lines:
//   2s→1s (Δℓ = 0, why 2s is metastable) and 3d→1s (Δℓ = −2).
#if SHOW_FORBIDDEN
let forbidden2sTo1s: DipoleAllowed.Type = Lowers<Orbital2s, Orbital1s>.self   // Δℓ = 0 — refused
let forbidden3dTo1s: DipoleAllowed.Type = Lowers<Orbital3d, Orbital1s>.self   // Δℓ = −2 — refused
#endif
