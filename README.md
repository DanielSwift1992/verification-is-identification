...
None of this uses `Int`, `String`, or Foundation in the system code. In the engine, a number is a type and a text is a type: coordinates, sizes, and glyph widths are named types the checker composes, and strings exist only as rendered output. The company alone counts 986 types, 0 structs, 0 classes, 0 runtime functions, and the engine and the theory count 0 runtime functions the same way. Every function that does execute belongs to the build tooling, counted separately, beside the system it serves.
