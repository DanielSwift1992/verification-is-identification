// ═══════════════════════════════════════════════════════════════
// Order Is Object (Strategy) — paper 8.
//
// V=I proved the agent's sole freedom is the order of COMPAREs (§5.25);
// that order carries zero identification bits at its own level (pure
...
/// permanent. [Order O9]
///
/// > Note: The order, once just transient scaffolding, freezes into a permanent
...
/// The ordering carries zero identification bits at its own level (InI S2, the
...
/// reach it fastest. Once ``PhaseBecomesAmplitude``, the object contains its own
...
/// > strategy matrix has its own clustering, and skills route only when it holds.
