// ═══════════════════════════════════════════════════════════════
// Interference Is Intelligence (Mechanism) — paper 5.
//
...
/// COMPARE resolves into three independent projections: Z, the outcome (one bit,
/// the ``Decidable`` verdict); X, the structure (the recursive form it walks to the
/// first divergence, ``OperationsClosed``); Y, the order (which COMPARE runs next,
/// ``OrderIsOnlyFreedom``). Z and X are fixed by the inputs; Y is the agent's choice
/// on a disjoint input, so the three are independent. [InI S1]
...
// they stay prose, as the E1 instrument stays in python/.
// ───────────────────────────────────────────────────────────────
