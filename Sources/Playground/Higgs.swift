...
/// A fully unbroken symmetry: no Goldstone direction, the count floors at `Never`. [Matter §5]
public enum Unbroken: BrokenSymmetry { public typealias Goldstones = Never }

/// One more broken generator adds one Goldstone direction (`Succ`). [Matter §5]
public struct Broken<Rest: BrokenSymmetry>: BrokenSymmetry { public typealias Goldstones = Succ<Rest.Goldstones> }

/// Electroweak breaking: three generators break, so three Goldstone directions
...
extension Broken: ThreeGoldstones where Goldstones == Three {}

/// A Goldstone count certified by the type checker: it forms only when the
/// symmetry breaks into exactly three directions. [Matter §5]
public enum Counted<Symmetry: ThreeGoldstones> {}
