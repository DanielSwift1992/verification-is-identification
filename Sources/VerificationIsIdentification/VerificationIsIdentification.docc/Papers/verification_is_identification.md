...
7. Finite data of bounded size lives in a finite space F. *[from 1 + 6: the structure has finite size n over its finite set of distinct components. All structures over those components up to size n form a finite set by combinatorics.]*
...
|----|-----------|------------|
| I1 | Every structure, and every set in play (atoms, F, S, G), is finite | From postulate: a finite structure has finitely many components; a bounded encoding generates finitely many structures (§0 step 7). |
...
- **NULL** is its fixpoint: the pair where left(x) = right(x) = x. Boundary is not a separate symbol — it is where distinction self-terminates. Base case detection: left(x) = x (self-reference), not x = ⊥ (external symbol).
...
1. **Finite basis.** The solution y can be assembled from a finite set of basis elements. *[from §0 steps 1, 6: finite structure → finite data → finite representation]*
...
The pipeline is linear: EXTRACT(unambiguous) → RESOLVE(ambiguous, preliminary) → REBUILD(all). Ambiguity resolution is a subroutine inside extraction, not a branch.
...
*Proof.* Every function in F maps output positions to input atoms (§3.2). If f ≠ g, they differ on some position p, selecting atoms i₁ ≠ i₂, so COMPARE(i₁, i₂) = not_equal. I1: N atoms → ≤ N² pairs. Each COMPARE classifies one pair. I3: classifications are permanent. After N² COMPAREs, all pairs classified. F is finite (§3.3). Any candidate verifiable against examples using cached results. |S| = 1 → exactly one passes → f* distinguished. QED
...
Theorems 1–2: soundness (PASS → correct). Theorem 5: completeness (preconditions → PASS). Extraction failure (§4.5–4.6) is a coverage limitation; f* remains certifiable.
...
**5.3. Cache monotonicity.** I3 → G_n ⊆ G_{n+1}. Cached comparisons are never invalidated. `work(game, G) = |needed| − |cached|` estimates uncached comparison work. Monotonically decreasing for overlapping domains.
...
**5.14. Graph equivalence at every node.** At any point in the computation, the remaining sub-graph from that node to any terminal state has the same V=I structure: finite remaining space, decidable check, append-only cache. *Proof.* I1 holds at every node (finite data processed → finite remaining). I2 holds (COMPARE unchanged). I3 holds (G at this node ⊇ G at start). All three invariants preserved. QED
...
**5.18. Cache transferability.** Graph G from solving game A accelerates game B if they share structures. *Proof.* COMPARE(a, b) = structural equality, context-independent. A comparison cached during game A is valid during game B (same structures → same result). I3 → G only grows. work(B, G_after_A) ≤ work(B, G_empty). QED

**5.19. Idempotency.** Running the same encoding on the same task twice produces the same result, with zero new comparisons on the second run. *Proof.* I2 (fixed operations): same input → same output. I3: second run finds all comparisons cached. Same result, zero new work. QED
...
**5.22. S is monotonically non-increasing.** Each COMPARE call can only eliminate candidates from S or leave S unchanged. No COMPARE call adds a candidate. *Proof.* S = {f ∈ F : f matches all examples}. A new COMPARE result adds to G (I3). More facts in G can only reveal that some f fails on some example → f leaves S. No new fact makes a previously failing f pass (the example it failed on is unchanged). |S_{n+1}| ≤ |S_n|. QED
