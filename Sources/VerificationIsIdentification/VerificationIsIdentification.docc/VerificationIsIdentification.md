...
- term **Check it:** <doc:AtlasOrder> puts every claim in an order where its premises come first, so a reader meets no claim before its premises. <doc:Atlas> ranks the same claims by what falls with them, <doc:AtlasAblation> names the premises the build needs (five of 2396), and <doc:AtlasRepeats> counts the premises a claim states twice.
