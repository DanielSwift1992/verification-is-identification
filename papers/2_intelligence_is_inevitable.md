...
**One game.** The agent submits encodings. The system checks. REJECT $\to$ modify $\to$ resubmit. Grammar is finite (I1). Resubmission of the same encoding is structurally futile: I3 + idempotency (V=I §5.19) guarantee the same result with zero new information. The set of untried encodings shrinks monotonically. Convergence: PASS or grammar exhausted.
...
| 1 | Positions: COMPARE(row(a), row(b)) | Per-encoding | Transfers. Same encoding $\to$ same positions. |
| 2 | Rules: COMPARE(rule(τ₁), rule(τ₂)) | Per-rule-class | Transfers. Same rule $\to$ same solution method. |
...
| T_min(τ) | Minimum COMPAREs to reach \|S\| $\le$ 1 on task τ. Property of the task |
| T_actual(P, τ) | Total COMPAREs protocol P uses to reach \|S\| $\le$ 1 on task τ |
| T_brute | Upper bound on T_actual for any task under brute-force enumeration. T_brute $\le$ N² (Theorem 3) |
...
η $\in$ (0, 1]. Shannon: any decision tree identifying all targets from |S₀| candidates has depth $\ge$ $\lceil$log₂|S₀|$\rceil$; per-target T_min can be shorter when a single feature distinguishes the target from many candidates. Bounded above: T_min $\le$ N² (Theorem 3). Computable (I1: finite space, finite decision trees). Substrate-independent. η is the natural measure here: the Saturation Theorem proves T_actual decreases, and η = T_min / T_actual directly reflects that decrease.
...
- For a task matching a stored rule: compare features of τ against each entry in L. Cost: O(|L| × F) COMPAREs. If match found $\to$ apply stored rule $\to$ compilation check: 1 pipeline execution. If PASS $\to$ done. Total: O(|L| × F) + 1.
- No match or compilation fails $\to$ full extraction. Cost: O(|L| × F) + T_brute.

Lookup cost is O(|L| × F), linear in library size and feature count. Both are finite (I1). For fixed D, |L| $\le$ K, so lookup is bounded by O(K × F), a constant for fixed D.

**Theorem (Saturation).** For any finite domain D under encoding E, let D_id $\subseteq$ D be the tasks for which |S| = 1 (identifiable under E). After at most K tasks (one per rule class in D_id), the agent's library contains a certified rule for every class in D_id. For all subsequent identifiable tasks in D: T_actual $\le$ O(K × F) + 1.

*Proof.* D_id is finite $\to$ K is finite. Each solved task whose rule is new adds one rule class to L (I3: append-only; §1: crystallization, each PASS is certified). After K tasks with distinct rules, every class in D_id is covered. For any future identifiable task τ from D, feature extraction + COMPARE against L finds a match (all classes covered). Apply stored rule. The rule is a structural predicate recomputed on τ's data (§2: why transfer works); since τ shares the same transformation as the original task in this class, compilation passes. T_actual = O(K × F) + 1. Tasks with |S| > 1 or |S| = 0 produce diagnostic output (§4: completeness boundary). QED
...
*Corollary (saturation cost).* Time to saturation $\le$ K × T_brute (K tasks at brute-force cost each). After that: effectively constant cost per task.

*Corollary (η after saturation).* After saturation, η(P, τ) = T_min(τ) / (O(K × F) + 1) for all τ in D_id. Since T_min $\le$ T_actual by definition, and both are bounded, η is well-defined and near-maximal when K × F is small relative to T_min.
...
*Remark (between-domain transfer).* If domains D₁ and D₂ share rule classes under the same encoding E, the agent's library from D₁ carries to D₂ for shared classes. The compilation gate verifies: if a rule from L₁ passes compilation on a D₂ task, it transfers. No new axiom needed — this is the Saturation Theorem applied to D₁ $\cup$ D₂.

**What the library stores.** Certified paths. Each entry passed compilation (Theorem 2) and is permanent (I3). Correctness of every entry in L is unconditional (Theorem 2: PASS $\to$ f' = f*).
...
**Encoding boundary.** No transfer between encodings. Different encoding $\to$ different positions $\to$ different features $\to$ level 1–2 cache is useless. On a new encoding, the agent starts from zero.
...
S₀ is a set of candidates. {Pₙ} is a sequence of decidable predicates on S₀, finite or infinite. Define Sₙ = {x $\in$ S₀ : P₁(x) $\land$ ... $\land$ Pₙ(x)} — the candidates surviving all predicates up to n.
...
|-------|------|-----------|
| A1 | Monotonicity | S₀ $\supseteq$ S₁ $\supseteq$ S₂ $\supseteq$ ... Each predicate can only exclude |
| A2 | Irreversibility | Pₙ, once imposed, is never retracted |
| A3 | Decidability | x $\in$ Sₙ is decidable for each n |
| A4 | Crystallization | $\exists$ n : \|Sₙ\| $\le$ 1 — the system reaches a terminal state |
...
*Proof.* S₀ = F, the finite space of candidates (I1). Each COMPARE result defines a predicate Pₙ: "candidate x is consistent with the n-th comparison." V=I §5.22: S is monotonically non-increasing. Each COMPARE can only eliminate candidates from S or leave S unchanged $\to$ A1. I3: the cache is append-only. A COMPARE result, once recorded, cannot be retracted $\to$ A2. I2: structural equality on finite structures is decidable $\to$ A3. Theorem 2 + R4: when |S| = 1 and PASS, f' = f*. For any finite deterministic task, an encoding producing |S| = 1 exists (Theorem 4), so the pipeline reaches |S| $\le$ 1 $\to$ A4. QED

**Lemma 2 (Agent learning).** Library accumulation under the Saturation Theorem is a crystallizing system.

A1 is defined as shrinkage: S₀ $\supseteq$ S₁ $\supseteq$ S₂ $\supseteq$ ... In Lemma 1 the shrinking object is obvious — S, the candidate set, loses members with each COMPARE. In §3, agent learning is described as L growing: rules are added, the library expands. But A1 requires something that shrinks. To apply the axioms, rewrite the same process from the other side: instead of counting what L has gained, count what L has not yet covered. That set — rule classes in D_id minus rule classes in L — is the one that shrinks. Every new PASS removes one class from it. It never grows back (I3). This is the object Lemma 2 uses as S₀.

*Proof.* S₀ = the set of rule classes in D_id not yet covered by L. This set is finite (I1 + finite D_id $\to$ finitely many rule classes). Each solved task whose rule is new defines a predicate Pₙ: "class k is covered." I3 for the agent (§2): entries are never removed from L, so a covered class stays covered. Uncovered classes can only decrease $\to$ A1. Covered classes cannot become uncovered $\to$ A2. Whether a new task matches an existing entry in L is decidable: compare features of τ against each stored rule, O(K × F) COMPAREs $\to$ A3. Saturation Theorem (§3): after K tasks with distinct rules, every class in D_id is covered. |uncovered| = 0 $\to$ A4 (terminal state: no candidates remain in S₀). QED
...
```
E[τ_sat] = K · Hₖ = K · Σᵢ₌₁ᴷ (1/i) $\approx$ K · ln(K) + γK
```

where Hₖ is the K-th harmonic number and γ $\approx$ 0.5772 is the Euler-Mascheroni constant.
...
*Proof.* Assume the task distribution has pₖ > 0 for all k (every class can appear). Uniqueness (§3) establishes endpoint invariance: the content of L depends on (D, E), not on the agent or task order. I3 establishes irreversibility: once a class enters L, it stays. The intermediate states L₁ $\subset$ L₂ $\subset$ ... $\subset$ L do depend on which tasks arrived first — agent A seeing class 3 before class 7 has a different L₄ than agent B who saw class 7 first. But both reach the same final L because every class in D_id eventually appears (pₖ > 0) and I3 prevents loss. The set of possible orderings is K! (all permutations of K classes). Each specific history selects one permutation. The endpoint is invariant across all K! paths. This is spontaneous symmetry breaking: the dynamics select one trajectory from a symmetric set, but the fixed point is unique. QED
...
**Corollary (Pre-existence).** f* exists before the agent starts. S = {y $\in$ F : passes verification} is determined by the task's structure and the encoding's test. The agent's COMPARE sequence does not construct f* — it eliminates every other candidate until f* is the only one left.
...
2. A function needs a caller $\to$ the loop $\to$ the agent (§0).
...
8. For finite D: L covers all K identifiable classes under E $\to$ T_actual drops to O(K × F) + 1 (§3, theorem). Tasks non-identifiable under E may resolve under a different encoding (§4: meta-saturation).
...
16. The crystallized library is not constructed by the agent but revealed. f* pre-exists. L is domain-determined. Search is elimination (§7).
