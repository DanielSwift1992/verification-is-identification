...
Distance Is Cost gave the library a metric: every pair of entries has a distance W, computable by one traversal. A metric space is not yet a map. This document derives the map. The library under W is a finite metric space; connecting entries within radius ρ makes it a graph (N1); the graph is connected exactly above a computable threshold (N2), and greedy descent toward a target terminates in at most W(start, target) hops because the metric is integer-valued and every hop strictly decreases it (N5). Where greedy descent fails, it fails at an address: a named node, a named target, a named radius — a hole that says where an anchor is missing (N6). Holes are repaired by insertion, repairs are permanent (I3), and inserting an anchor never creates a hole where none was (N15–N16): the router's failures are the acquisition curriculum (N17). Above the unconditional core sit the conditional theorems, in the same standing as the modulus and the compression criterion: a library whose geometry is hierarchically separated is navigable at small radius in logarithmic hops (N11–N12), the cost of building the map amortizes over queries with a computable crossover (N13), and a populating library navigates inevitably, for the same reason intelligence is inevitable — append-only accumulation over a finite clustered space (N19). Learning, generalization, and navigation turn out to be three readings of one measurable property of the W-matrix (N18). The map is not added to the library. It was implied by the metric; this document only draws it.
