...
**B8e (Measurement — two residue curves, sharpening §5).** §5's single residue curve splits in two: |atoms|(s) [the volume] and N(s) [the behavioral surface]. Both bounded ⇒ CLOSED; |atoms| plateaus while N(s) → ∞ ⇒ HORN (the jackpot: finite priors, unbounded recombination — "an evaluation set that defeats r = 0" is, formally, *a horn*); |atoms| grows ⇒ PRIMES (B8a). The log-slope of N(s) measures γ. Sampling caveat for self-play: on an infinite-reach horn no uniform measure over ⟨Π⟩ exists, and any norm-discount β^{‖g‖} normalizes only for β < e^{−γ}, concentrating on small norm — so coverage must be enumerated by norm, never sampled (Play Is Proof PP5c).
...
## §5. Measurement: Phase 2 of E1, Extended

The clustering measurements of E1 Phase 2 (N(r), δ, ρ_nav) test whether the rule space *has* geometry. The sieve tests the sharper thing: whether the rule space has a *finite generating basis*, and whether that basis *generalizes from train to eval* — which is precisely Chollet's design question (eval requires novel recombination of priors) rendered as a measurement.

**Setup.** From the certified library (E1 Phase 1), with composition defined on the DSL: program composition is functional composition of solvers (Hodel's DSL programs compose by construction). ‖·‖ is the AST norm (E1 Phase 0). Cl_d is bounded-depth composition search, gated.

**Procedure.**

1. *Run the sieve (B3)* over the 400 training solvers in norm order, at depth bounds d = 1, 2, 3, … . Output: the atom set A_d and the DERIVED/ATOM label of every rule.
2. *The residue curve.* Plot |atoms discovered| against |library scanned|, in norm order, for each d. Plot the *discovery rate* (new atoms per 10 tasks) against position.
3. *The generalization test.* Build A_d from all 400 train solvers. For each of the 400 eval tasks, test whether its certified rule ∈ Cl_d(A_d) — reachable by composing train atoms. Fraction reachable = the basis's coverage of eval.
4. *The two-norm check (B4).* For a sample of composite (DERIVED) rules, confirm ‖R‖_comp < ‖R‖ (a real shortcut exists); for atoms, confirm ‖R‖_comp ≥ ‖R‖ at the reported depth. This is the prime-characterization, unit-tested on rules.

**What the curves mean — declared before measurement:**

| Observation | Reading | Source |
|---|---|---|
| Discovery rate → 0 as library grows; |A_d| plateaus | finite basis; domain saturable; recomposition dominates | B7, B8 |
| Discovery rate stays bounded away from 0; |A_d| grows ≈ linearly with library | open basis; prime-like; each task brings irreducible structure | B8a |
| Eval coverage (step 3) high at small d | train atoms generate eval — ARC is recombination of a finite basis (Chollet's bet wins for the learner) | B6, B7 |
| Eval coverage low at all affordable d | eval needs atoms absent from train — the basis does not transfer (ARC defeats this grammar's learner) | N18, IiI §4 |
| |A_d| shrinks sharply as d increases | deep composition compresses the basis — few primitives, much recombination | B9 depth-relativity |
| |A_d| barely moves with d | the basis is genuinely broad, not a depth artifact | B9 |

**The fork is symmetric and both branches are results.** A finite, transferring basis is the strongest possible positive: ARC is the depth-d closure of a small atom set, and the learner that finds the atoms solves the domain. An infinite-looking, non-transferring basis is a clean negative with a precedent (the primes): ARC under this grammar is open — irreducible structure keeps arriving — established by the residue curve, not asserted. Either way, the *size and transfer of the atom basis* is the single most decision-relevant number the project can produce, and it is computable from data already in hand.

**On 50 of 400, runnable now.** The sieve does not wait for the full library. Run B3 on the 50 certified solvers: the early residue curve already shows a slope. A curve already bending toward plateau at 50 is evidence (preliminary, so labeled) that the basis is small; a curve climbing one-atom-per-task at 50 is evidence it is broad. The 50-task residue is a real measurement of the basis's local slope — the first number on the question that subsumes saturation, clustering, and generalization at once.

## §6. The Open Question, Named

**B11 (Computable, not yet compressible).** The sieve computes the basis (B3, terminating by I1). This is *constructive* definability: to know the atoms up to norm n, run the sieve to norm n. It is not *compressed* definability: no method is known to predict the residue more cheaply than running the sieve. The two are different (the framework's standing distinction, G12: a path computed is not a path compressed). For primes, this gap is the prime-distribution problem: the sieve computes them, the prime-number theorem compresses their *density* (≈ n/ln n — a found axis for the statistics), but their *exact sequence* has no known compression — the Riemann Hypothesis is the question of whether a deeper regularity axis exists. For a rule domain the identical gap holds: §5 measures the residue; whether the residue has a generating formula cheaper than the sieve is open per domain. The framework does not solve this. It *locates* it precisely — as the compressibility of a sieve's residue — and refuses the mysticism: the residue is not random (B10), it is determined-but-possibly-incompressible, which is a sharp and honest status.
...
7. The sieve is an executable addition to E1 Phase 2: the residue curve on 50→400 ARC solvers, and the train-atom coverage of eval, measure basis size and transfer — the number that subsumes saturation, clustering, and generalization (§5).
