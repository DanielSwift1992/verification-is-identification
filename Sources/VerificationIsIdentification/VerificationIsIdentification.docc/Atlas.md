...
The table is testable. Delete any claim's declaration from the source and build: the compiler lists every claim that falls, and the length of that list is this table's Carries number. The table and the compiler read the same source, so the two counts cannot drift apart. <doc:AtlasAblation> runs that test over every premise on this page: each premise is cut, the module is rebuilt, and the table there records which cuts the build fails.
