...
// ═══════════════════════════════════════════════════════
// Markers — functions on spaces, rooted in primitives.
//
...
/// Convergence to one: all distinctions resolve. `|S| = 1`.
///
...
/// > Identification *is* this collapse; ``VI_Identity`` names the same point
/// > from the solution-set side.
public protocol Unique: Monotone, Null {}

/// Accumulation: after ⊇ before. It only grows.
///
...
public protocol Permanent: Pair where Right: Pair, Right.Left == Left {}

/// Elimination: after ⊆ before. Shrinks toward ``Null``. Mirror of ``Permanent``.
///
...
public protocol Monotone: Pair where Left: Pair, Left.Left == Right {}
...
public protocol MetricProperty: Pair where Left: Pair, Right: Pair, Left.Right == Right.Left {}
...
public protocol IntegerValued: Pair where Right: IntegerValued {}

/// Computable from structure alone: input maps to one determined value.
///
/// The output side settles — `Right` is ``Null``, collapsed to a point. One
/// input, one answer, fixed by the structure. [GA GA3]
public protocol Measurable: Pair where Right: Null {}
...
// ═══════════════════════════════════════════════════════
// Axes — degrees of freedom that theorems constrain.
//
...
/// Σ: the space of all structures. The root axis every other axis sits inside.
///
...
/// The solution set: the candidates that survive verification.
///
/// A sub-space of ``HasSigma``. Verification shrinks it; ``VI_Identity`` pins it
...
/// The library: accumulated certified knowledge. A sub-space of ``HasSigma``.
///
...
/// The comparison cache: every `COMPARE` result, recorded. A sub-space of ``HasSigma``.
///
...
/// Distances between structures in Σ. A sub-space of ``HasSigma``.
///
...
/// The encoding: the agent's perspective on the target. A sub-space of ``HasSigma``.
///
...
/// α, the alignment coefficient, computed from distance: `α = f(W)`.
///
...
/// Basis of the library: its generating set. A sub-space of ``HasLibrary``.
///
...
public protocol VI_Identity: Null, HasSolutions where Left == SolutionSet {}
