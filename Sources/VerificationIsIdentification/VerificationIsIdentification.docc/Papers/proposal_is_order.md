...
No new postulate. Everything below is derived from the three axes of COMPARE (InI §0), order as the sole free variable (V=I §5.25), the confusion-cover (InI S5b), alignment (GA), and the Saturation Theorem (IiI §3).
...
2. Two operations, **disjoint inputs**: the test outputs Z (the cut) and X (the frontier) from the candidate and target; the proposal — which candidate next — is the Y-axis, a function of the agent's state, independent of any specific outcome (InI S1). *[axis independence]*
...
4. solve = iterate(D ∘ G). The series so far is the theory of D (V=I = the certified cut; IiI = its inevitable optimization). G was delegated. This document is the theory of G and its relation to D. *[the gap]*

## §1. The Two Organs

**GD1 (The discriminator is the cut; the generator is the order).** D is the Z-axis: each test eliminates survivors (V=I §5.22) and is the sole source of identification facts (§5.21). G is the Y-axis: choosing the next candidate carries zero new bits (InI S2) but sets the total test count. Information enters only through D; G only chooses where D looks.
...
*The inversion.* The organ carrying all the information (D) is distinct from the organ whose choices we call "search" (G). This is the centre of the document: difficulty and information sit on opposite organs.
...
**GD3 (Discriminate-easy and generate-easy are independent — InI S1 lifted, GA7 completed).** The two terrains do not determine one another; all four quadrants are inhabited.
...
*GA7 named two generator-axes (clustering, alignment); GD3 adds the discriminator axis (separation) and proves it independent of both.*

## §3. The Asymmetry of Inevitability

**GD4 (The inevitability is the discriminator's).** "Intelligence is inevitable" (IiI §3) is a theorem about D: at saturation the library teaches the confusion structure, so the optimal ordering becomes computable and η → 1 (InI S6). G's efficiency is conditional — it rides on clustering / modulus (which improve with the library only if the domain clusters, NiG N19) and on alignment α (which saturation never changes, GA).
...
(ii) *G is conditional on α.* The loss landscape loss_τ(R) = Σ_k W(apply(R, I_k), O_k) (DiC G13) is a function of (candidate R, task τ, encoding E). The alignment coefficient α = corr(W(R, R*), loss_τ(R)) (GA3) is computed over this function. Neither loss_τ(R) nor W(R, R*) depends on the library L: L records which candidates have been tested, not how untested candidates perform. Therefore α is invariant under accumulation at a fixed encoding. Saturation grows L (the agent's knowledge of the landscape); it does not reshape the landscape itself. G beats enumeration iff α > 0 (GA6); no amount of saturation can supply this. QED.
...
**GD8 (Both organs are COMPARE; the split is scope — the framework stays monist).** G and D are not two mechanisms. Both are COMPARE (V=I §2.5). D is COMPARE producing the cut (Z); G is the choice of which COMPARE next (Y). The split is a split of *scope* on one primitive, exactly as InI's three axes are projections of one COMPARE. The agent does not *have* a generator and a discriminator; its single act of comparison, read for its outcome, is the discriminator, and read for its order, is the generator. *(Consistent with PP15: both organs run on the one given COMPARE; neither certifies it.)*
...
On a horn (B8b: K = ∞, finite basis, infinite reach): (1) holds — the gate is per-task, independent of K. (2) fails — the confusion matrix over infinitely many classes is never fully learned, so the discriminator's *learned optimality* (InI S6) is unreachable. (3) is the only available path: compose from the certified basis (B6–B7), certify each result per-task via the gate. The horn strips intelligence to *generation from a certified basis with per-task certification* — no lookup intelligence remains.

*Proof.* (1) Theorem 2 is per-task, order- and history-independent (V=I §5.25): it needs only the gate and a candidate, not the library. (2) IiI §3 (finiteness-of-classes remark): the Saturation Theorem requires K < ∞; on a horn K = ∞ by B8b. The confusion matrix has infinitely many rows; no finite library stores it. (3) The basis A is finite (B8b); its closure generates all reachable rules (B6). With no complete library to look up, the agent must compose from A and submit each composition to the gate — generation is forced (PP6: the endpoint is the basis, not the library). QED.

*Corollary (the horn efficiency measure).* On a horn, η = T_min/T_actual (IiI §3) is undefined for library-lookup (no finite library exists to measure against). The relevant efficiency is the generator's: how many compositions from A does the agent try before the gate certifies? This count is bounded below by the compositional depth of the solution (B4: the shortest composition from atoms) and depends on α (GA6: aligned → descent through composition space; rugged → enumeration of compositions). The horn's difficulty is entirely a generator question.

## §5. Boundaries

**Per-encoding.** α (G's terrain) and the confusion structure (D's terrain) are both encoding-relative (DiC §11): a re-encoding can move difficulty between the organs. The split is invariant; the location of difficulty is per encoding.

**Conditionality is not pessimism.** Where the domain is aligned and clustered, G is as inevitable as D (N19). The asymmetry bites only on rugged / unclustered domains — and it bites exactly there because that is where α = 0.
...
**Stochastic / partial.** The gate's certificate weakens on stochastic or partially-observed domains (LiT §6); both organs inherit the weakening.

## §6. Statement

1. A solve is discriminate ∘ generate; the two have disjoint inputs (S1 lifted to organs).
2. The discriminator is the cut (Z, all the information); the generator is the order (Y, none) — difficulty and information on opposite organs (GD1).
3. Two geometries — confusion-cover and loss-over-W — independent: all four easy/hard quadrants inhabited (GD3, completing GA7).
4. The inevitability is the discriminator's; the generator is conditional on α, which saturation cannot manufacture (GD4) — and generator-power is exactly ordering-visibility, exactly α (GD5).
5. Two failures, two ledger terminals; E2's encoder/backend is this split; both organs are one COMPARE at different scope (GD6–GD8).
...
> **Solving has two organs: one proposes, one cuts. The cutting carries all the information and is the half the series proved inevitable; the proposing carries none and rides on a geometry no learning can supply. Intelligence-as-selection is a theorem; intelligence-as-search is a wager on alignment. The hard questions and the easy ones live on different organs — and difficulty sits, as ever, where information does not.**
...
Verification Is Identification proved what happens once a candidate is in hand: the cut is certified, and at saturation the cutting is optimal. This document proves what was always delegated — where the candidate comes from — and proves it perpendicular to the cut: a generator that rides on alignment, a discriminator that rides on the gate, two axes of the one act of comparison, hard and easy on opposite sides.
