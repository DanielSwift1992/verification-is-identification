...
V=I proves the agent's sole degree of freedom is the order of comparisons (§5.25). Interference Is Intelligence proves this order carries zero new bits at its own level — phase, not amplitude (S2). Intelligence Is Inevitable records the consequence as a caveat: knowledge inherits I3, but "the agent's strategy state does not — it is mutable and resets between runs" (IiI §2). This document removes the caveat by changing its level. A strategy over a finite domain is a finite decision tree; a finite decision tree is a structure in Σ; structures in Σ can be targets of V=I games. Therefore strategies can be verified, identified, certified, and crystallized — they enter the library as permanent objects (O9), carrying the metric, the balls, and the navigation of the later papers with them (O13–O15). The ordering that is phase at level N is amplitude at level N+1 (O10). The completed library entry becomes a triple — rule, encoding, strategy: WHAT the answer is, THROUGH WHICH lens, and HOW to reach it fastest (O12). The path itself, not merely its endpoint, becomes a certified object — the top rung of the objectification ladder (O17), and "skill" acquires a formal definition: a ball in strategy space (O18).
