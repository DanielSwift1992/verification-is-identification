...
// ═══════════════════════════════════════════════════════════════
// A certified scheduler — the V=I loop as a working tool.
//
...
/// A pinned task: a given. Its machine is fixed, the seed propagation grows from. [V=I §3]
public struct Pinned<M: Machine>: Task { public typealias Assigned = M }
...
extension Conflict: Separated where V.Assigned == U.Assigned.Opposite {}

/// A satisfied constraint, certified by the type checker: it forms only when its
/// conflict edge separates its endpoints. [V=I Thm 1]
public enum Require<Edge: Separated> {}

// The conflict graph — a 4-cycle of tasks, T0 pinned to machine A. Propagation
...
public struct CrystallizedSchedule: SystemCrystallizes {
    ...
    public typealias Left        = Cell<Never, Never>
    public typealias Right       = Cell<Never, Never>
    public typealias Cache       = Grew<Never, Never>   // resolved conflicts ACCUMULATE (I3)
...
/// |S| > 1: the SAME graph with `T0` left free admits two assignments (swap A↔B).
...
public struct AmbiguousSchedule {
    ...
    public typealias Left        = Cell<Never, Never>
    public typealias Right       = Cell<Never, Never>
    public typealias Cache       = Grew<Never, Never>
    public typealias SolutionSet = Cell<Three, Never>   // |S| = 2 — not Unique
...
#if SHOW_UNSAT
public typealias ImpossibleConflict = Require<Conflict<T0, T2>>   // A vs A — cannot separate
#endif
#if SHOW_AMBIGUOUS
