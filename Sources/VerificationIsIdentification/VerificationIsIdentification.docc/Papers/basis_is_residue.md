...
Intelligence Is Inevitable proves the library saturates after one task per rule class — K tasks. This document sharpens K. Most rule classes are not new: they are recompositions of structure already certified. The new entries — those no recomposition of prior entries reproduces — are the domain's *atoms*, defined exactly as the prime numbers are, by survival of a sieve.
